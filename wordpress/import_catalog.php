...
foreach ($products as $item) {
    ...
    $price = 0;
    // Extract first occurrence of price (sale price often first if listed)
    if (preg_match('/R\$\s*([\d\.,]+)/u', $price_html, $matches)) {
        // Convert "1.234,50" to "1234.50"
    ...
    $product->set_stock_quantity(100); // Default stock
    $product->set_description("Produto importado de toysport.com.br\nLink original: " . $item['url']);
